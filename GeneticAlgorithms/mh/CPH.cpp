...
int CPH::reservarMemoriaInstancia() {
	...
	data.nodos=new struct nodo [data.n];
...
float CPH::evaluarSolucion(struct datosSolucionCPH *sol) {
	...
	for(int j=0;j<data.p;j++){
		for(int i=0;i<data.n;i++){
			if((sol->x[i]==servidor || sol->x[i]==(data.p+servidor)) && data.demandaV[j]+data.nodos[i].demanda<data.c) {
				data.demandaV[j]=data.demandaV[j]+data.nodos[i].demanda;
			}
		}
		servidor++;
	 }
...
struct datosSolucionCPH * CPH::generarSolucionRandom() { 
	...
	int cap [data.p];
    for(int i=0;i<data.p;i++)cap[i]=data.c;
...
for(int i=0;i<data.n;i++){
        ...
        if(!encuentra){
                    while(!concentrador){
                          s=rand()%data.p;
                          if(cap[s]-data.nodos[i].demanda<0){concentrador=false;}
                          else{
                                concentrador=true;
                                cap[s]-=data.nodos[i].demanda;
                                sol->x[i]=s;
                               }
                            }
...
struct datosSolucionCPH * CPH::aplicarAlgoritmoGenetico(int tamanoPoblacion, float pCruce, float pMutacion, int nEval, bool estacionario)
{
	...
	if(estacionario) 
	{
		do{
			...
			for(int i=0;i<data.n;i++){
				aux[tamanoPoblacion]->x[i]=pob2[0]->x[i];
			}
			aux[tamanoPoblacion]->aptitud=pob2[0]->aptitud;

			for(int i=0;i<data.n;i++){
				aux[tamanoPoblacion+1]->x[i]=pob2[1]->x[i];
			}
			...
			pob2=seleccionPorTorneo(aux,2,(tamanoPoblacion+2),tamanoPoblacion-1);
	...
	{
		do{
			struct datosSolucionCPH * auxmejor=mejorSolucionPoblacion(pob, tamanoPoblacion); 
			...
			pob2=seleccionPorRuleta(pob, tamanoPoblacion, tamanoPoblacion-1);
			for (int i=0;i<tamanoPoblacion-1;i++){
				r = ((double)rand())/RAND_MAX;
			...
			for(int j=0;j<data.n;j++){
				pob[tamanoPoblacion-1]->x[j]=mejor->x[j];
			}
			pob[tamanoPoblacion-1]->aptitud=mejor->aptitud;
